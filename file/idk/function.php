<?php
    ...
    function createuser($conn, $firstname, $lastname, $email, $phone_number, $password1, $city){
        require_once "login.dbh.php";
        $error = "";
        $user_existed = user_exists($conn, $email, $phone_number);
        if($user_existed == NULL){
            $sql = "INSERT INTO user(firstname, lastname, email, phone_number, password1, city) VALUES(?,?,?,?,?,?);";
            $smt = mysqli_stmt_init($conn);
            if(!mysqli_stmt_prepare($smt, $sql)){
                $error .= "Error in STMT";
                exit();
            ...
            mysqli_stmt_bind_param($smt, "sssiss", $firstname, $lastname, $email, $phone_number, $hashedPwd, $city);
            mysqli_stmt_execute($smt);
            mysqli_stmt_close($smt);
            $error .= "User Created Successfully";
            return $error;
    ...
    function login($conn, $email, $password){
        ...
        if($user_existed === false){
            ...
            $pwdhashed = $user_existed["password1"];
